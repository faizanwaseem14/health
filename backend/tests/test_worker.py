...
stub) is also tested here in isolation - run_ocr_for_report itself
(the actual OCR + evidence-storage work) is tested for real in
test_ocr_service.py and the Task summary's scratch-Postgres run.
"""
...
def test_process_ocr_job_returns_completed_when_words_are_found():
    ...
    with (
        ...
        patch("app.jobs.worker.run_ocr_for_report", return_value=fake_result),
    ):
