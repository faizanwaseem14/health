...
def test_process_ocr_job_returns_completed_when_extraction_and_trust_checks_succeed():
    ...
    with (
        ...
        patch("app.jobs.worker.resolve_aliases_for_report") as mock_resolve,
        patch("app.jobs.worker.apply_unit_conversion_for_report") as mock_convert,
    ):
        outcome = process_ocr_job(job)

    assert outcome == COMPLETED
    mock_extract.assert_called_once_with(fake_db, fake_report, job.id)
    mock_resolve.assert_called_once_with(fake_db, fake_report.id)
    mock_status.assert_called_once_with(fake_db, fake_report.id)
    mock_convert.assert_called_once_with(fake_db, fake_report.id)
